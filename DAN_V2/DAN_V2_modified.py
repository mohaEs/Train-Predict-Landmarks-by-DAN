...
def video_input_fn(path_img,img_size,num_lmark):
# video = cv2.VideoCapture(data_dir)
    ...
    def _get_frame():
        ...
        #path_img = 'prepared_data/aa_a.png'
...
def main(argv):
    ...
    if flags.mode == tf.estimator.ModeKeys.PREDICT:
        ...
        img_path,pts_path = get_filenames(flags.data_dir)
        #print(img_path)
        for counter in range(len(img_path)):   
            path_img = img_path[counter]
            print('*********** Predict:')
            input_function = video_input_fn(path_img,112,flags.num_lmark)
            dan_run_loop_modified.dan_main(flags,vgg16_model_fn,input_function, file_path=path_img)
        return
    print('*********** Train:')
